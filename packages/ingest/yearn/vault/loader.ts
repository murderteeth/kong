...
import db, { toUpsertIfAsOfSql } from '../../db'
    ...
    this.worker = mq.worker(mq.q.yearn.vault.load, async job => {
      switch(job.name) {
        case mq.q.yearn.vault.loadJobs.vault: {
          ...
          const query = toUpsertIfAsOfSql('vault', 'chain_id, address', vault)
          ...
          for(const item of withdrawalQueue) {
            ...
            const query = toUpsertIfAsOfSql('withdrawal_queue', 'chain_id, vault_address, queue_index', item)
